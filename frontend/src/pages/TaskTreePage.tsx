    ...
    <div className="flex h-full">
      <div className="flex-1 overflow-auto p-6">
        ...
        {(!tree?.tree || tree.tree.length === 0) ? (
          <div className="card p-8 text-center text-fg-dim">
            No nodes yet. Click <strong>Add Part</strong> or use the MCP <code className="font-mono">create_node</code> tool.
          </div>
        ) : (
          <div className="space-y-1">
            {tree.tree.map((part: any) => (
              <NodeRow
                ...
                toggle={toggle}
                selected={selected}
                onSelect={(nid) => { setSelected(nid); setSearchParams({ step: nid }) }}
      ...
      {createOpen && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4">
          <div className="card w-full max-w-md p-5">
            <h2 className="mb-3 text-lg font-medium">Create {createOpen.type}</h2>
            {createOpen.parent && (
              <div className="mb-3 text-xs text-fg-muted">Under: {createOpen.parent.title}</div>
            )}
            ...
            <input className="input mb-2 w-full" placeholder="Title *" value={createForm.title} onChange={e => setCreateForm({ ...createForm, title: e.target.value })} />
            <textarea className="input mb-2 w-full" rows={3} placeholder="Description" value={createForm.description} onChange={e => setCreateForm({ ...createForm, description: e.target.value })} />
            <textarea className="input mb-2 w-full" rows={2} placeholder="Acceptance criteria" value={createForm.acceptance_criteria} onChange={e => setCreateForm({ ...createForm, acceptance_criteria: e.target.value })} />
            ...
            <input className="input mb-3 w-full" placeholder="Reason (why are we adding this?)" value={createForm.reason} onChange={e => setCreateForm({ ...createForm, reason: e.target.value })} />
            <div className="flex gap-2">
              <button className="btn-primary" onClick={submitCreate}>Create</button>
              <button className="btn" onClick={() => setCreateOpen(null)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
...
function NodeRow({ node, depth, expanded, toggle, selected, onSelect, openCreate, matchesFilter }: any) {
  const isExpanded = expanded.has(node.id)
  const hasChildren = (node.children || []).length > 0
  const visible = matchesFilter(node)
  const filteredChildren = (node.children || []).filter((c: any) => true)
  // collapse if filter excludes node and no descendant matches
  ...
  return (
    <>
      ...
      >
        ...
        {icon}
        <button
          className={`flex-1 truncate text-left ${textSize}`}
        ...
        {node.node_type === 'part' && (
          <button title="Add Phase" className="btn px-1.5 py-1 text-xs" onClick={() => openCreate(node, 'phase')}>
            <Plus size={12} />
          </button>
        )}
        {node.node_type === 'phase' && (
          <button title="Add Step" className="btn px-1.5 py-1 text-xs" onClick={() => openCreate(node, 'step')}>
            <Plus size={12} />
          </button>
        )}
      </div>
